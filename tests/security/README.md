...
| --- | --- | --- |
| `test_routes.py` | 1 | Both metadata documents: 200, `resource` == `WIKI_BASE_URL + "/mcp"` byte for byte, exactly one `authorization_servers` entry (AS-2), no `scopes_supported` (ADR-0016 withdrew custom scopes). `OPTIONS /mcp` unauthenticated with CORS headers exposing `WWW-Authenticate` and allowing the MCP header set. `GET`/`DELETE /mcp` → 405. `POST /mcp` without a token → 401 with `Bearer resource_metadata="<metadata URL>"`, no `scope`, and `Access-Control-Expose-Headers` on the gateway response itself (§9.3). `/`, `/health`, `/metrics`, `/admin`, `/mcp/x`, `/.well-known/nope`, a random path → 404 with no "Missing Authentication Token", no traceback, no request echo. |
