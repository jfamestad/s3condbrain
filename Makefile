...
grant-owner: ## Bootstrap the first owner: make grant-owner SUBJECT=user_xxx EMAIL=you@x TABLE=...
	uv run python scripts/grant_owner.py --bootstrap --subject $(SUBJECT) --email $(EMAIL) --name "$(NAME)" --table $(TABLE)
